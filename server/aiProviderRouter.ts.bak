...
// サポートするAIプロバイダー
export type AIProvider = 'gpt-3.5' | 'gpt-4' | 'gemini-pro';
...
async function analyzeWithOpenAI(caption: string, model: 'gpt-3.5-turbo' | 'gpt-4'): Promise<any> {
  const prompt = `
...
// Gemini Proで分析
async function analyzeWithGemini(caption: string): Promise<any> {
...
// メイン関数
export async function analyzePost(userId: string, post: PostInput): Promise<any> {
  // プロバイダーを決定（フロントエンドから送信されたもの、またはデフォルト）
  const provider: AIProvider = (post.aiProvider as AIProvider) || getDefaultProvider();
