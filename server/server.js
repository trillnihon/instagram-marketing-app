...
// Facebook Login for Business コールバックエンドポイント
app.post('/auth/facebook/callback', async (req, res) => {
  const { access_token, long_lived_token, expires_in, data_access_expiration_time, redirect_uri } = req.body;
  ...
  const logStep = (step, message, data = null) => {
    ...
    // デバッグモードの場合のみログを出力
    if (isDebugMode) {
      console.log(`🎯 [FACEBOOK CALLBACK STEP ${step}] ${message}`, data ? data : '');
      console.log(`⏰ [FACEBOOK CALLBACK STEP ${step}] タイムスタンプ: ${timestamp}`);
    }
  };
  
  logStep(1, 'Facebook Login for Business コールバック開始');
  console.log('[DEBUG] Facebookコールバック - 受信したaccess_token:', access_token ? access_token.substring(0, 10) + '...' : 'なし');
  console.log('[DEBUG] Facebookコールバック - 受信したlong_lived_token:', long_lived_token ? long_lived_token.substring(0, 10) + '...' : 'なし');
  
  if (!access_token) {
    logStep(2, 'アクセストークン不足 - エラーレスポンス');
    return res.status(400).json({ error: 'アクセストークンが必要です' });
  }
  
  logStep(3, 'アクセストークン検証完了');
  
  try {
    logStep(4, 'Facebook API処理開始');
    
    // ユーザー情報取得
    logStep(5, 'Facebookユーザー情報取得開始');
    ...
    logStep(9, 'Instagramビジネスアカウント検索開始', { pageCount: pages.length });
    
    for (const page of pages) {
      logStep(10, `ページ確認中: ${page.name}`, { 
        pageId: page.id,
        hasInstagramAccount: !!(page.instagram_business_account && page.instagram_business_account.id)
      });
      ...
      if (page.instagram_business_account && page.instagram_business_account.id) {
        ...
        logStep(11, 'Instagramビジネスアカウント発見', {
          instagramId: instagramBusinessAccount.id,
          username: instagramBusinessAccount.username,
          pageName: instagramBusinessAccount.page_name
        });
    ...
    if (!instagramBusinessAccount) {
      logStep(12, 'Instagramビジネスアカウントが見つからない - 警告レスポンス');
      console.warn('[WARNING] Instagramビジネスアカウントが見つかりません');
      
      // Instagramビジネスアカウントがなくても、Facebook認証は成功とする
      res.json({
        success: true,
        warning: 'Instagramビジネスアカウントが見つかりません',
        access_token: access_token,
    ...
    // Instagram投稿データ取得（最新5件）
    logStep(13, 'Instagram投稿データ取得開始');
    let recentPosts = [];
    ...
    try {
      ...
      logStep(14, 'Instagram投稿データ取得成功', { 
        postCount: mediaRes.data.data?.length || 0 
      });
      
      recentPosts = mediaRes.data.data || [];
    } catch (mediaError) {
      logStep(14, 'Instagram投稿データ取得失敗 - 警告として処理継続', { 
        error: mediaError.response?.data || mediaError.message 
      });
      console.warn('[WARNING] Instagram投稿データ取得失敗:', mediaError.response?.data || mediaError.message);
      // 投稿データが取得できなくても認証は成功とする
    }
    
    // 成功レスポンス
    logStep(15, 'Facebookコールバック処理完了 - 成功レスポンス送信');
    res.json({
      success: true,
      access_token: access_token,
      long_lived_token: long_lived_token || access_token,
    ...
    logStep(16, 'Facebookコールバック処理でエラー発生', {
      error: err.response?.data || err.message,
      status: err.response?.status,
      errorCode,
      errorSubcode,
    ...
    console.error('[ERROR] Facebookコールバック処理失敗:', debugInfo);
    
    // Graph API エラーコード別の詳細メッセージ
    let errorMessage = 'Facebook認証失敗';
    if (errorCode === 190) {
      errorMessage = 'アクセストークンが無効です。再認証が必要です。';
    } else if (errorCode === 191) {
      errorMessage = 'リダイレクトURIが許可されていません。Facebook設定を確認してください。';
